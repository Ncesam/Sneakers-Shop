        ...
        ({ item }) => {
          return (
            <View style={{ marginBottom: 15, marginRight: 15 }}>
              <Card id={item.id} cost={item.cost} name={item.name} isFavorite />
            </View>
          )
        }
      } keyExtractor={(item, _) => item.id.toString()} numColumns={2} style={styles.itemsScrollContainer} contentContainerStyle={{ marginBottom: 50 }}>
      </FlatList>
    </View>
  )
}
